...
[The VLESS transport protocol](https://xtls.github.io/en/development/protocols/vless.html) is used on xray to proxy client traffic through the remote server without tunnelling. The VLESS protocol delivers UDP traffic by creating TCP streams. A TCP stream is created for every 1024 UDP 4-tuple and is kept alive until the connection had been idle for 300 seconds. The 4-tuple is source address, source port, destination address, and destination port. The XUDP feature is used, which puts a 16-24 bytes header on each TCP stream carrying UDP traffic. This header provides address and port of the responding host to support full-cone NAT behaviour. MPTCP is enabled on xray client outbound and server inbound. Therefore, client TCP and UDP traffic is delivered to the remote server using MPTCP. Xray on the remote server then uses TCP or UDP to proxy the client traffic to the target host.
...
- Deploy the website to GitHub Pages to https://fs.bondingshouldbefree.org/.
...
- `bsbf-client-installer.sh`:
  - The BSBF bonding solution client installer for conventional Linux distributions.
    ...
    - TCP port 8080 and 12345 listening on localhost.
    - UDP port 12345 listening on localhost.
    - Packet marks 1 and 2.
    - Metrics 9 to 16 on default routing table.
    - Routing table 1.
- `bsbf-client-openwrt-installer.sh`:
  - The BSBF bonding solution client installer for OpenWrt.
    ...
    Resources the BSBF bonding solution client will use:
    - TCP port 12345 listening on localhost.
    - UDP port 12345 listening on localhost.
    - Packet marks 1 and 2.
    - Metrics 1 to [WAN Interface Count, max. 8] and 9 to 16 on default routing table.
    - Routing table 1.
- `bsbf-mptcp`:
