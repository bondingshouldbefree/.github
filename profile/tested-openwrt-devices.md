...
- Mikrotik first time OpenWrt image installation procedure: Download an initramfs image for the device. Run the [mikrotik-netboot.sh](include/mikrotik-netboot.sh) script as root on a Linux distro with dnsmasq installed. Turn off distro's network profile for the network interface so it won't interfere with the IP addressing of the interface.
